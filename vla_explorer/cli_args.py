...
def update_rsl_rl_cfg(agent_cfg: RslRlOnPolicyRunnerCfg, args_cli: argparse.Namespace):
    ...
    if args_cli.use_cnn is not None:
        agent_cfg.use_cnn = args_cli.use_cnn
        agent_cfg.policy.class_name = "ActorCriticDepthCNN"
        agent_cfg.policy.obs_depth_shape = (24, 32)
    if args_cli.use_rnn:
        agent_cfg.policy.rnn_input_size = 2 * agent_cfg.policy.actor_hidden_dims[-1]
        agent_cfg.policy.rnn_hidden_size = 2 * agent_cfg.policy.actor_hidden_dims[-1]
        if not args_cli.use_cnn:
            agent_cfg.policy.class_name = "ActorCriticRecurrent"
        else:
            agent_cfg.policy.class_name = "ActorCriticDepthCNNRecurrent"
            
    agent_cfg.policy.history_length = args_cli.history_length
