...
parser.add_argument("--task", type=str, default="Go2Vlnce-MP3D-Base-vision-v0", help="Name of the task.")
...
from envs.vlnce.robots import *
from envs.vlnce.utils import ASSETS_DIR, RslRlVecEnvHistoryWrapper, VLNEnvWrapper
...
class Planner:
    ...
    def start_loop(self):
        ...
        robot_yaw_quat = math_utils.yaw_quat(
            self.env.unwrapped.scene["robot"].data.root_quat_w[0].detach().cpu()).unsqueeze(0)
        ...
        while self.simulation_app.is_running():  # 20hz
            ...
            relative_pos = torch.tensor(env_cfg.expert_path[self.expert_vel_idx] - robot_pos_w,
                                        device=self.env.unwrapped.device).to(torch.float32)
            expert_pos_body_frame = math_utils.quat_rotate_inverse(
                math_utils.yaw_quat(self.env.unwrapped.scene['robot'].data.root_quat_w),
            ...
            target_yaw = self.compute_target_yaw(env_cfg.expert_path[self.expert_vel_idx], env_cfg.expert_path[
                min(self.expert_vel_idx + 1, env_cfg.expert_path_length - 1)])
...
if __name__ == "__main__":
    ...
    with gzip.open(dataset_file_name, "rt") as f:
        ...
        if "go2" in args_cli.task:
            env_cfg.scene.robot.init_state.pos = (episode["start_position"][0], episode["start_position"][1],
                                                  episode["start_position"][2] + 0.4)
        elif "h1" in args_cli.task:
            env_cfg.scene.robot.init_state.pos = (episode["start_position"][0], episode["start_position"][1],
                                                  episode["start_position"][2] + 1.0)
        else:
            env_cfg.scene.robot.init_state.pos = (episode["start_position"][0], episode["start_position"][1],
                                                  episode["start_position"][2] + 0.5)
        # 设置标记物的位置
        env_cfg.scene.disk_1.init_state.pos = (episode["start_position"][0], episode["start_position"][1],
                                               episode["start_position"][2] + 2.5)
        # 终止点上方
        env_cfg.scene.disk_2.init_state.pos = (episode["reference_path"][-1][0], episode["reference_path"][-1][1],
                                               episode["reference_path"][-1][2] + 2.5)
        # 设置机器人的初始旋转
    ...
    log_root_path = os.path.join(os.path.dirname(__file__), "../logs", "vlnce_isaac", agent_cfg.experiment_name)
    log_root_path = os.path.abspath(log_root_path)
    # import pdb; pdb.set_trace()
    resume_path = get_checkpoint_path(log_root_path, args_cli.load_run, agent_cfg.load_checkpoint)

    ppo_runner = OnPolicyRunner(env, agent_cfg.to_dict(), log_dir=None,
                                device=agent_cfg.device)  # Adjust device as needed
